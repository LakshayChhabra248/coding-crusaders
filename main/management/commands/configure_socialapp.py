...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not client_id or not client_secret:
            self.stdout.write(self.style.ERROR('Env vars GOOGLE_CLIENT_ID and GOOGLE_CLIENT_SECRET must be set'))
            return
        ...
        if created:
            self.stdout.write(self.style.SUCCESS('SocialApp created and attached to Site'))
        elif changed:
            self.stdout.write(self.style.SUCCESS('SocialApp updated'))
        else:
            self.stdout.write(self.style.SUCCESS('SocialApp already configured'))
